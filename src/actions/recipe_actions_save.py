    ...
    def run(
        ...
        if isinstance(steps_slot, list) and steps_slot:
            steps = steps_slot
             ...
             # Fallback similar to ActionTellRecipeStep
            for slot_name in ("recipe_card", "recipe_json", "last_recipe", "recipe"):
                raw = tracker.get_slot(slot_name)
                if raw is None:
                    continue
                if isinstance(raw, dict):
                    ...
                    continue
                recipe = data.get("recipe") if isinstance(data, dict) else None
                s = recipe.get("steps") if isinstance(recipe, dict) else None
        ...
        if not steps:
            dispatcher.utter_message(
                text="Je n'ai pas de recette en mémoire pour la lire en entier."
            )
            return []

        # Build the full text
        lines = []
        lines.append(f"Voici la recette complète :")
        ...
        for i, step in enumerate(steps):
            ...
            # Retrieve timer if any
            timer_min = step.get("timer_min")
            timer_str = ""
            try:
                t_int = int(timer_min)
                if t_int > 0:
                    timer_str = f" ({t_int} min)"
            except:
                pass

            lines.append(f"Étape {idx}: {instruction.strip()}{timer_str}")
